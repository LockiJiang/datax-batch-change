...
public class Configuration {
    ...
    public static Configuration from(File file) {
        try {
            ...
            throw new RuntimeException(String.format("配置信息错误. 您提供配置文件[%s]读取失败，错误原因: %s. 请检查您的配置文件的权限设置.", file.getAbsolutePath(), e));
        }
    }
    ...
    public Character getChar(final String path) {
        ...
        try {
            ...
            throw new RuntimeException(String.format("任务读取配置文件出错. 因为配置文件路径[%s] 值非法，期望是字符类型: %s. 请检查您的配置并作出修改.", path, e.getMessage()));
        }
    }
    ...
    public Boolean getBool(final String path) {
        ...
        if (null == result) {
            ...
            throw new RuntimeException(String.format("您提供的配置信息有误，因为从[%s]获取的值[%s]无法转换为bool类型. 请检查源表的配置并且做出相应的修改.", path, result));
        }
    ...
    public Integer getInt(final String path) {
        ...
        try {
            ...
            throw new RuntimeException(String.format("任务读取配置文件出错. 配置文件路径[%s] 值非法, 期望是整数类型: %s. 请检查您的配置并作出修改.", path, e.getMessage()));
        }
    }
    ...
    public Double getDouble(final String path) {
        ...
        try {
            ...
            throw new RuntimeException(String.format("任务读取配置文件出错. 配置文件路径[%s] 值非法, 期望是浮点类型: %s. 请检查您的配置并作出修改.", path, e.getMessage()));
        }
    }
    ...
    private void setObject(final String path, final Object object) {
        ...
        throw new RuntimeException(String.format("值[%s]无法适配您提供[%s]， 该异常代表系统编程错误, 请联系DataX开发团队!", ToStringBuilder.reflectionToString(object), path));
    }
    ...
    Object buildObject(final List<String> paths, final Object object) {
        ...
        for (int i = paths.size() - 1; i >= 0; i--) {
            ...
            throw new RuntimeException(String.format("路径[%s]出现非法值类型[%s]，该异常代表系统编程错误, 请联系DataX开发团队! .", StringUtils.join(paths, "."), path));
        }
        return child;
    }
